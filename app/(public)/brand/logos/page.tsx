    ...
    <>
      ...
      <div className="py-10 sm:py-16">
        <article className="mx-auto max-w-3xl px-4 sm:px-6">
          <h1 className="font-display text-3xl font-bold tracking-tight sm:text-4xl">{t('brandLogos.title')}</h1>
          ...
          <ol className="mt-10 list-decimal space-y-4 pl-5 text-sm text-muted-foreground leading-relaxed">
            {(['brandLogos.step1', 'brandLogos.step2', 'brandLogos.step3', 'brandLogos.step4'] as const).map((key) => (
              <li key={key}>{t(key)}</li>
            ))}
          </ol>
          ...
          <section className="mt-12">
            ...
            <ul className="mt-4 grid gap-2 sm:grid-cols-2">
              {CUSTOMER_LOGOS.map((logo) => (
                <li key={logo.id} className="rounded-lg border border-border/50 bg-muted/20 px-3 py-2 font-mono text-xs">
