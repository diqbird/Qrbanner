#!/usr/bin/env python3
"""Deploy Critical Growth Pack v1: hero, trust, sticky CTA, new pages."""
import os
import sys
import paramiko
...
_, o, _ = c.exec_command(
    f"cd {REMOTE} && yarn build > /tmp/qrb-growth1.log 2>&1; echo EXIT:$?; tail -12 /tmp/qrb-growth1.log",
    timeout=600,
)
out = o.read().decode("utf-8", errors="replace")
log_path = os.path.join(LOCAL, "scripts", "_deploy_growth1.txt")
with open(log_path, "w", encoding="utf-8") as f:
    f.write(out)
_, o, _ = c.exec_command(f"cd {REMOTE} && pm2 restart qrbanner 2>/dev/null; echo done", timeout=30)
c.close()
if "EXIT:0" not in out:
    print("BUILD FAILED - see scripts/_deploy_growth1.txt")
    sys.exit(1)
print("deploy ok")
