  ...
  return (
    <PremiumPageFrame narrow="3xl">
      ...
      <header className="text-center">
        <h1 className="ph-title text-4xl sm:text-5xl">{headline}</h1>
        <p className="ph-lead mx-auto mt-4 max-w-2xl">{description}</p>
      ...
      {sections.map((section) => (
        <section key={section.title} className="mt-14">
          <h2 className="ph-title text-xl">{section.title}</h2>
          {section.ordered ? (
      ...
      {faqTitle && faqItems && faqItems.length > 0 ? (
        <section className="mt-14" aria-labelledby="programmatic-faq">
          <h2 id="programmatic-faq" className="ph-title text-xl">
            {faqTitle}
          </h2>
      ...
      <div className="mt-12 rounded-xl border border-primary/20 bg-primary/5 p-8 text-center">
        <h2 className="ph-title text-xl">{ctaTitle}</h2>
