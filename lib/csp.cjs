...
function buildContentSecurityPolicy(options = {}) {
  ...
  // Styles: React `style={{}}` needs style-src-attr 'unsafe-inline'.
  // Framer Motion / Crisp inject <style> tags without nonces — keep elem unsafe-inline.
  // Do not add a style nonce yet: CSP3 would ignore 'unsafe-inline' and break those tags.
  const styleSrc =
